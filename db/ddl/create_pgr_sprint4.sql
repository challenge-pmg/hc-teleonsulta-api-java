CREATE TABLE T_TDSPW_PGR_USUARIO (
    ...
    role VARCHAR2(20) CHECK (role IN ('ADMIN','PACIENTE','PROFISSIONAL')) NOT NULL,
...
CREATE TABLE T_TDSPW_PGR_PACIENTE (
    ...
    cidade VARCHAR2(100),
    status VARCHAR2(20) CHECK (status IN ('ATIVO','INATIVO')) DEFAULT 'ATIVO',
    CONSTRAINT fk_paciente_usuario FOREIGN KEY (id_usuario) REFERENCES T_TDSPW_PGR_USUARIO(id_usuario)
);
...
CREATE TABLE T_TDSPW_PGR_PROFISSIONAL_SAUDE (
    ...
    status VARCHAR2(20) CHECK (status IN ('ATIVO','INATIVO')) DEFAULT 'ATIVO',
    CONSTRAINT fk_profissional_usuario FOREIGN KEY (id_usuario) REFERENCES T_TDSPW_PGR_USUARIO(id_usuario),
    CONSTRAINT fk_profissional_tipo FOREIGN KEY (id_tipo_profissional) REFERENCES T_TDSPW_PGR_TIPO_PROFISSIONAL_SAUDE(id_tipo_profissional)
);
...
CREATE TABLE T_TDSPW_PGR_CONSULTA (
    ...
    tipo_consulta VARCHAR2(20) CHECK (tipo_consulta IN ('PRESENCIAL','TELECONSULTA')) NOT NULL,
    link_acesso VARCHAR2(255),
    status VARCHAR2(20) CHECK (status IN ('AGENDADA','REALIZADA','CANCELADA','FALTOU')) DEFAULT 'AGENDADA',
    criado_em TIMESTAMP DEFAULT SYSTIMESTAMP,
    CONSTRAINT fk_consulta_paciente FOREIGN KEY (id_paciente) REFERENCES T_TDSPW_PGR_PACIENTE(id_paciente),
    CONSTRAINT fk_consulta_profissional FOREIGN KEY (id_profissional) REFERENCES T_TDSPW_PGR_PROFISSIONAL_SAUDE(id_profissional),
    CONSTRAINT fk_consulta_agendador FOREIGN KEY (id_usuario_agendador) REFERENCES T_TDSPW_PGR_USUARIO(id_usuario)
);
...
CREATE TABLE T_TDSPW_PGR_MENSAGEM_AUTOMATICA (
    ...
    CONSTRAINT fk_mensagem_consulta FOREIGN KEY (id_consulta) REFERENCES T_TDSPW_PGR_CONSULTA(id_consulta)
);
...
CREATE TABLE T_TDSPW_PGR_FEEDBACK_CONSULTA (
    ...
    criado_em TIMESTAMP DEFAULT SYSTIMESTAMP,
    CONSTRAINT fk_feedback_consulta FOREIGN KEY (id_consulta) REFERENCES T_TDSPW_PGR_CONSULTA(id_consulta)
);
...
CREATE TABLE T_TDSPW_PGR_ACESSO_TELECONSULTA (
    ...
    CONSTRAINT fk_acesso_consulta FOREIGN KEY (id_consulta) REFERENCES T_TDSPW_PGR_CONSULTA(id_consulta)
);
...
SELECT 
    ...
    c.tipo_consulta,
    c.status,
    CASE WHEN c.tipo_consulta = 'TELECONSULTA' THEN c.link_acesso ELSE NULL END AS link_acesso
